...
from ..database.models3 import Base, Game3, GamePlayer3, Player3, PlayerGameTypeStats3
from ..config3 import (
    GameMode,
    GAME_GAP,
    ONE_WEEK,
...
def compute_remaining_game_player_stats(game_player: GamePlayer3, playtime: int, is_mvp: bool, is_lvp: bool) -> Dict[str, Any]:
...
def calculate_game_player_rating(game_type: GameMode, game_player: GamePlayer3, player_stats: PlayerGameTypeStats3, player_average_stats: dict, team_elo, team_glicko, game_players_to_insert) -> int:
    total_avg_deltas = {}
...
"""
def update_player_game_type_stats(player_stats: PlayerGameTypeStats3, game_player: GamePlayer3) -> None:
...
def simulate_player_game_type_stats(game_type: GameMode, ref_players_ids: List[int]) -> None: 
    ...
    for player in session.query(Player3).all():
...
def simulate_game_mode_games(game_type: GameMode, ref_players_ids: List[int], env) -> None:    
    ...
    for ref_player_id in ref_players_ids: # This is how we test each scenario (every player is every scenario in every combination)
        if ref_player_id != 1:
            for player_game_type_stat in session.query(PlayerGameTypeStats3).all():
                player_game_type_stat.last_time_played = GLOBAL_START_TIME
        # Retrieve player's party name and stats
        current_ref_player = session.query(Player3).filter_by(id=ref_player_id).first()
        ...
        if current_ref_player.party_name != f"Party_{ref_player_id}":
            ...
            if "half" in current_ref_player.party_name:
                for _ in range(game_type.group_sizes[0] - 1):
                    player_party.append(session.query(Player3).filter_by(id=next_player_id).first())
        ...
        for (ref_rating_coeficient, ref_games_count, party_coeficient, time_gap, k_factor) in REF_COEF_AND_GAMES[f"player_{ref_player_id}"]:
            ...
            for _ in range(ref_games_count):
                ...
                player_stats = session.query(PlayerGameTypeStats3).filter_by(
                ...
                party_players_stats = session.query(PlayerGameTypeStats3).filter(
                    PlayerGameTypeStats3.game_type == game_type.type,
                    PlayerGameTypeStats3.player_id.in_(player_party_ids),
                ).all()
                ...
                game_players_stats = session.query(PlayerGameTypeStats3).filter(
                    PlayerGameTypeStats3.true_rating.between(search_rating - 50.0,
                                                              search_rating + 50.0),
                    PlayerGameTypeStats3.game_type == game_type.type,
                    PlayerGameTypeStats3.player_id.notin_(exclude_ids),
                    or_(PlayerGameTypeStats3.last_time_played <= filter_time),
                ).order_by(
                    func.rand()
                ...
                if len(game_players_stats) < (player_count - len(player_party)):
                    game_players_stats = session.query(PlayerGameTypeStats3).filter(
                        PlayerGameTypeStats3.true_rating.between(search_rating - 100.0,
                                                                  search_rating + 100.0),
                        PlayerGameTypeStats3.game_type == game_type.type,
                        PlayerGameTypeStats3.player_id.notin_(exclude_ids),
                        or_(PlayerGameTypeStats3.last_time_played <= filter_time),
                ...
                for ref_player in party_players_stats:
                    ...
                    game_players_to_insert.append(
                        GamePlayer3(
                            ...
                            team="Team_1",
                            party_name=ref_player.player3.party_name,
                    ...
                    for _ in range(slots_to_fill):
                        ...
                        game_players_to_insert.append(
                            GamePlayer3(
                                ...
                                team=team_name,
                                party_name=team_player.player3.party_name,
                ...
                for game_player in game_players_to_insert:
                    ...
                    game_player_game_type_stats = session.query(PlayerGameTypeStats3).filter(
                        PlayerGameTypeStats3.player_id == game_player.player_id,
                    ).first()
                    player_stats_calculated = compute_remaining_game_player_stats(game_player, playtime, is_mvp, is_lvp)

                    new_game_player = GamePlayer3(
                        created_at=current_time,
                        game3=game,
                ...
                for game_player in game_players_to_insert:
                    p_stats = session.query(PlayerGameTypeStats3).filter_by(
...
def simulate_all_modes() -> None:
    players_to_create = []
    ref_players_ids = list(range(3, REFERENCE_PLAYER_COUNT + 1))
    logger.info("Creating players...")
    
    created_player_count = session.query(Player3).count()
        ...
        for player_id_index in range(1, TOTAL_PLAYERS + 1):
            ...
            for id_range, name in SCENARIO_PLAYER_PARTIES:
                if player_id_index in id_range:
                    party_name = name
                    break
            players_to_create.append(Player3(id=player_id_index, name=f"Player_{player_id_index}", party_name=party_name))
